...
def _confidence_settings(strategy: StrategyConfig) -> dict[str, object]:
    ...
    return {
        "enabled": enabled,
        "min_scale": float(conf.get("min_scale", 0.8)),
        "max_scale": float(conf.get("max_scale", 1.2)),
        "clip_z": float(conf.get("clip_z", 3.0)),
        "long_only": bool(conf.get("long_only", False)),
        "dd_floor": float(conf.get("dd_floor", 0.8)),
        "dd_ceiling": float(conf.get("dd_ceiling", 1.1)),
    }

def _crowd_penalty_settings(strategy: StrategyConfig) -> dict[str, object]:
    cfg = strategy.portfolio or {}
    cp = cfg.get("crowd_penalty", {}) if isinstance(cfg, dict) else {}
    enabled = bool(cp.get("enabled", False))
    return {
        "enabled": enabled,
        "lookback_days": int(cp.get("lookback_days", 30)),
        "power": float(cp.get("power", 1.0)),
        "target": cp.get("target", "signal"),  # "signal" or "vol"
    }

def _hold_decay_settings(strategy: StrategyConfig) -> dict[str, object]:
    cfg = strategy.portfolio or {}
    hd = cfg.get("hold_decay", {}) if isinstance(cfg, dict) else {}
    return {
        "hold_min": int(hd.get("hold_min_rebalances", 0)),
        "decay_alpha": float(hd.get("decay_alpha", 0.0)),  # blend prev and new weights: w = a*prev + (1-a)*new
    }
...
def run_backtest(
    ...
    cp_params = _crowd_penalty_settings(strategy)
    ...
    position_age: Dict[str, int] = {}  # rebalance periods in current sign
    universe_history: List[UniverseSelection] = []
    results: List[dict] = []
    returns_history: List[float] = []
    ...
    prop_cooldown = 0
    prop_halted = False
    returns_window_w = None  # avoid unbound

    universe_pointer = 0
    current_universe: List[str] = []
    ic_history: List[float] = []
    hd_params = _hold_decay_settings(strategy)
    ...
    while True:
        ...
        # Optional inversion (e.g., mean-reversion)
        if bool(strategy.signal.get("invert", False)) and not signal_frame.empty:
            signal_frame = signal_frame.copy()
            signal_frame["signal"] = -signal_frame["signal"]
        # Optional crowd penalty: down-weight highly correlated names
        if cp_params["enabled"] and returns_window_w is not None and not signal_frame.empty:
            syms = [s for s in signal_frame["symbol"] if s in returns_window_w.columns]
            sub = returns_window_w[syms].dropna(how="any")
            if not sub.empty:
                corr = sub.corr()
                mean_corr = corr.mean().clip(lower=0)  # keep positive part
                penalty = (1.0 + mean_corr) ** cp_params["power"]
                signal_frame = signal_frame.copy()
                for sym in syms:
                    if sym in penalty.index:
                        signal_frame.loc[signal_frame["symbol"] == sym, "signal"] /= penalty[sym]
        # Rolling returns windows
        returns_window_beta = None
        if beta_neutral_cfg:
            ...
            if not sub.empty:
                ...
                if not piv.empty:
                    returns_window_beta = piv
        returns_window_w = None
        lookback_w = int(strategy.portfolio.get("vol_lookback_days", 30))
        window_start_w = rebalance_date - pd.Timedelta(days=lookback_w)
        sub_w = df[(df["date"] > window_start_w) & (df["date"] <= rebalance_date)]
        if not sub_w.empty:
            piv_w = sub_w.pivot_table(index="date", columns="symbol", values="close").pct_change().dropna(how="all")
            if not piv_w.empty:
                returns_window_w = piv_w
        ...
        if prop_enabled and prop_halted:
            ...
            if long_short_enabled:
                ...
                weights = portfolio.compute_long_short_weights(
                    ...
                    returns_window=returns_window_beta if weighting != "max_div" else returns_window_w,
                )
            else:
                weights = portfolio.compute_weights(
                    ...
                    weighting=weighting,
                    volatility_col="volatility" if eff_vol_col == vol_col else "vol_liq",
                    returns_window=returns_window_w,
            ...
            # Hold-min and decay blending
            if weights:
                # hold-min: prevent flips before hold_min rebalances
                hold_min = hd_params["hold_min"]
                if hold_min > 0:
                    new_w = {}
                    for sym, w in weights.items():
                        prev_w = prev_weights.get(sym, 0.0)
                        age = position_age.get(sym, 0)
                        if prev_w != 0 and np.sign(prev_w) != 0 and np.sign(w) != np.sign(prev_w) and age < hold_min:
                            new_w[sym] = prev_w
                        else:
                            new_w[sym] = w
                            if np.sign(w) != np.sign(prev_w):
                                position_age[sym] = 0
                    # keep aging entries that are not in new_w
                    for sym in list(position_age.keys()):
                        if sym not in new_w and sym in prev_weights and position_age[sym] < hold_min:
                            new_w[sym] = prev_weights[sym]
                        elif sym not in new_w:
                            position_age.pop(sym, None)
                    weights = new_w
                # decay blend with previous weights
                if hd_params["decay_alpha"] > 0 and prev_weights:
                    a = hd_params["decay_alpha"]
                    blended = {}
                    symbols = set(prev_weights) | set(weights)
                    for sym in symbols:
                        blended[sym] = a * prev_weights.get(sym, 0.0) + (1 - a) * weights.get(sym, 0.0)
                    gross_new = sum(abs(v) for v in blended.values())
                    gross_target = sum(abs(v) for v in weights.values())
                    if gross_new > 0 and gross_target > 0:
                        scale = gross_target / gross_new
                        blended = {k: v * scale for k, v in blended.items() if abs(v) > 0}
                    weights = blended
            # Confidence scaling |signal|/vol
            if conf_params["enabled"] and weights:
                scales = []
                sig_map = signal_frame.set_index("symbol")["signal"]
                vol_map = eligible.set_index("symbol")[vol_col]
                for sym, w in list(weights.items()):
                    if sym in sig_map and sym in vol_map:
                        sig = float(sig_map.loc[sym])
                        volv = float(vol_map.loc[sym])
                        if volv > 0:
                            z = min(abs(sig) / volv, conf_params["clip_z"])
                            sc = conf_params["min_scale"] + (conf_params["max_scale"] - conf_params["min_scale"]) * (z / conf_params["clip_z"])
                            # drawdown dampening
                            sc *= max(conf_params["dd_floor"], min(conf_params["dd_ceiling"], 1.0 - drawdown))
                            if conf_params["long_only"] and w < 0:
                                sc = 1.0
                            scales.append(sc)
                            weights[sym] = w * sc
